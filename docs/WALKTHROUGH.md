# Complete Project Walkthrough — MovieRS

> A step-by-step guide explaining every decision, every component, and every error faced while building this production-grade Movie Recommendation System — from scratch to AWS deployment.
...
1. **No user data needed** — works from day one
2. **Explainable** — I can say "recommended because both are sci-fi thrillers"
3. **No cold-start** — new movies can be recommended immediately if they have metadata

### Limitations
- Cannot capture personal taste (user who loves "bad" horror movies won't get them)
- No social signal ("your friends watched this")
- Limited to metadata quality — if TMDB overview is vague, similarity suffers
...
```
┌──────────────┐     ┌──────────────────────────────────────┐
│   Browser    │────▶│  Nginx (Reverse Proxy + SPA Server)  │
...
| `overview` | "A thief who steals corporate secrets..." | **Primary ML feature** — most text for similarity |
...
"sci-fi, action/adventure!" → "scifi actionadventure"

# Step 3: Remove stopwords
"a thief who steals from the mind" → "thief steals mind"

# Step 4: Combine features
overview + genre_names + keywords → single text blob
```
...
| **IDF** (Inverse Document Frequency) | How rare this word is across all movies | "heist" appears in only 5 out of 5000 movies → high IDF |
| **TF-IDF** | TF × IDF | "heist" is both frequent in Inception AND rare overall → HIGH score |

### Why TF-IDF Over CountVectorizer?

| Method | Problem |
|--------|---------|
| CountVectorizer | Treats all words equally — "movie" (appears everywhere) gets same weight as "heist" (rare, meaningful) |
| TF-IDF | Downweights common words, upweights distinctive words |

### How Movies Become Vectors

```
Movie: "Inception" → text: "thief steals dreams heist subconscious mind"
                   → TF-IDF vector: [0.0, 0.45, 0.0, 0.62, 0.0, 0.38, ...]
                                      ↑              ↑              ↑
                                   "action"       "heist"        "mind"
...
Score = 1.0 → identical direction (same content)
Score = 0.0 → completely different content
```

### Why Cosine Over Euclidean Distance?

Euclidean distance is affected by document length — a 500-word overview vs a 50-word overview would be "far apart" even if they discuss the same topics. Cosine similarity only cares about the **direction** (what topics), not the **magnitude** (how many words).
...
- **Without precompute**: Each request recalculates similarity → 2-5 seconds per request
- **With precompute**: Load matrix from file → <50ms per request
...
User types "inception" in search bar
    → Frontend sends: GET /api/v1/search?q=inception&page=1
    → Nginx proxies to Flask backend
    → Flask route handler:
        1. Check Redis cache (key: "search:inception:1")
        2. Cache miss → Query SQLite FTS5 index
...
```python
STALE_TRENDING = 6 * 3600      # 6 hours — changes fast
STALE_POPULAR = 12 * 3600      # 12 hours
STALE_HIGH_RATED = 7 * 86400   # 7 days — very stable
STALE_GENRES = 30 * 86400      # 30 days — almost never changes
```

When data is stale, the system fetches fresh data from TMDB, saves it, and serves the new version. If TMDB is down, it serves the stale data with `source: "database_stale"` — degraded but functional.
...
## 10. AWS Deployment — Step by Step
...
|--------|-----------|-------------|
| CPU Utilization | Every 5 min | EC2 → Instance → Monitoring tab |
| Network In/Out | Every 5 min | Same location |
| Disk Read/Write | Every 5 min | Same location |
| Status Checks | Every 1 min | EC2 → Instance → Status checks |

**How to view:** AWS Console → EC2 → Instances → Select `MovieRecommender` → **Monitoring** tab
...
**Fix (Development):** Vite proxy handles this — frontend sends requests to itself, Vite forwards to backend.

**Fix (Production):** Not an issue — Nginx serves both frontend and API from the same origin (port 80).

### Error 8: Gunicorn Workers × APScheduler Duplicates
...
| Low | CI/CD pipeline (GitHub Actions → ECR → ECS) | 1 day |
...
>
> The system uses a cache-aside pattern — Redis for hot cache, SQLite for persistence, TMDB API as the source of truth. Each data category has its own staleness threshold. The React frontend is served by Nginx which also reverse-proxies API requests to the Flask backend. The whole thing runs as three Docker containers on a single EC2 instance."

### 5-Minute Explanation

Add to the above:

> "For data preprocessing, I combine movie overviews with genre names, lowercase everything, remove stopwords, and strip special characters. This cleaned text is vectorized using TF-IDF which weights words by how unique they are to each movie — so 'heist' gets a high score for Inception because it's rare across all movies but frequent in Inception's text.
>
...
> Content-based filtering doesn't have a user cold-start problem — new users get recommendations based on the movie they're viewing. For movie cold-start, any movie with an overview can be vectorized immediately. APScheduler pre-warms trending data so the database is never empty.

**Q: Why SQLite instead of PostgreSQL?**
> For a single-instance deployment, SQLite provides ACID compliance with zero infrastructure overhead. The entire database is a single file that persists via a Docker volume. The code is abstracted so migrating to PostgreSQL would only require changing the database module — all route and service code stays the same.

**Q: What happens if TMDB API goes down?**
> The system has 4 levels of fallback: Redis → SQLite → TMDB API → default_popular table. If the API is unreachable, it serves stale data from the database with a `source: "database_stale"` tag. If even the database is empty (first boot), it returns a hardcoded fallback. The system never returns a 500 error for data availability issues.
...
> Without precomputation, every recommendation request would compute cosine similarity across the entire TF-IDF matrix — O(n²) operation taking 2-5 seconds. With precomputation, it's a single array lookup — O(1) in under 50ms. The tradeoff is memory, but for ~5000 movies the matrix is only ~100MB.

**Q: Explain your caching strategy.**
> Cache-aside pattern with two layers. Redis (in-memory, TTL-based) handles hot data for sub-millisecond responses. SQLite (persistent, staleness-based) survives container restarts. Each category has its own staleness threshold — trending data becomes stale after 6 hours, genre lists after 30 days. This prevents unnecessary API calls while keeping data fresh where it matters.

---

*Built by Deepanshu Talan — [GitHub](https://github.com/deepanshu-talan/scalable-movie-recommendation-system)*
